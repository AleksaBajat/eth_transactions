...
#
#info("Average price of gas in gwei : {}".format(average_price))
#
#
...
#info("Average price of transaction in euro: {}".format(average_tp))
#
...
#
#info("Failed transactions : {}%".format(round(ratio * 100, 4)))
#
#
## AVERAGE GAS USED BY TRANSACTION
#
#agt_df = (df.select(Columns.GAS_UNITS).agg(savg(Columns().GAS_UNITS))).collect()[0][0]
#info("Average gas used up by transaction: {}".format(round(agt_df, 2)))
#
#
...
#
#info("Smart contract interaction percentage: {}%".format(round(ratio * 100, 4)))
#info("Simple transfer percentage: {}%".format(round((100 - ratio) * 100, 4)))
#
#
